...
def logo():
    figlet = Figlet(font="standard").renderText("WDZONE CC")
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by WDZONE Team ")
print (gn + "[+] Made By WOODcraft")
print (cy + "[=] WDZONE CC Tools Version : 1.1")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Channel : @Opleech" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
if opr == "1":
    ...
    print (mag + "[$] Telegram Channel : @Opleech" + cv)
